...
class ParticleFiler():
    ...
    def lidarCB(self, msg):
        ...
        if not isinstance(self.laser_angles, np.ndarray):
            ...
            rospy.loginfo(self.downsampled_angles.shape[0])
    ...
    def initialize_global(self):
        ...
        '''
        rospy.loginfo("GLOBAL INITIALIZATION")
    ...
    def sensor_model(self, proposal_dist, obs, weights):
        ...
        if self.RANGELIB_VAR == VAR_RADIAL_CDDT_OPTIMIZATIONS:
            if "cddt" in self.WHICH_RM:
                ...
                rospy.loginfo("Cannot use radial optimizations with non-CDDT based methods, use rangelib_variant 2")
            ...
            if self.SHOW_FINE_TIMING and self.iters % 10 == 0:
                rospy.loginfo(
                    f"sensor_model: init: {((t_init - t_start) / t_total):.2f} "
                    f"range: {((t_range - t_init) / t_total):.2f} "
                    f"eval: {((t_eval  - t_range) / t_total):.2f} "
                    f"squash: {((t_squash - t_eval) / t_total):.2f}"
                )
            ...
            rospy.loginfo("PLEASE SET rangelib_variant PARAM to 0-4")
    ...
    def MCL(self, a, o):
        ...
        if self.SHOW_FINE_TIMING and self.iters % 10 == 0:
            rospy.loginfo(
                f"MCL: propose: {((t_propose - t)      / t_total):.2f} "
                f"motion: {((t_motion  - t_propose) / t_total):.2f} "
                f"sensor: {((t_sensor  - t_motion)  / t_total):.2f} "
                f"norm:   {((t_norm    - t_sensor)  / t_total):.2f}"
            )
        # save the particles
...
def load_params_from_yaml(fp):
    ...
    with open(fp, 'r') as infile:
        ...
        for param in yaml_data:
            rospy.loginfo(f"param: {param}: {yaml_data[param]}")
